...
def notificar_evento(ev):
    ...
    if es_reconexion:
        # Mensaje corto de reconexion (el evento ya tiene hora de fin real).
        texto = (
        ...
        # En reconexion, el detalle suele traer la hora de sincronizacion; lo
        # incluimos porque aporta (ej. "a las 18:56 h sincronizo con el SEIN").
        if ev.get("detalle"):
            texto += f"\n\nℹ️ *Detalle:* {ev['detalle']}"
    else:
        # Mensaje completo de desconexion (TRIP).
        texto = (
            f"🔔 *{cabecera}*\n"
            f"🏭 Empresa: {ev['empresa']}\n"
            f"📍 Ubicacion: {ev['ubicacion']}\n"
