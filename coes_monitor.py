...
# Si True, ademas de ORYGEN, reporta eventos de cualquier empresa cuya SUMA de
# potencias (leidas del detalle, en MW) supere UMBRAL_MW. Si False, solo ORYGEN.
REPORTAR_OTRAS_EMPRESAS = True
# Umbral de potencia en MW. Eventos de terceros cuya SUMA de MW del detalle sea
# >= a este valor se reportan. Cambia el numero a gusto, o pon
# REPORTAR_OTRAS_EMPRESAS = False para apagar el radar de terceros.
UMBRAL_MW = 100.0

# --- Deteccion de centrales propias en el texto del detalle (Cambio 3) ---
# Si el detalle de un evento (aunque sea de otra empresa) menciona alguna de
# estas centrales, se reporta SI O SI, sin importar empresa ni potencia.
# Se busca cada palabra clave como substring, sin distinguir mayus/minus ni
# tildes. Usa los nucleos de nombre (sin "C.H."/"S.E.") para captar variantes.
CENTRALES_PROPIAS = [
    "Huampani",     # Huampani / Huampaní
    "Moyopampa",
    "Callahuanca",
    "Huinco",
    "Matucana",
    "Chimay",
    "Yanango",
    "Wayra",        # cubre Wayra I, Wayra II, Wayra Extension
    "Rubi",         # Rubi / Rubí
    "Clemesi",      # Clemesi / Clemesí
    "Santa Rosa",
    "Ventanilla",
]
...
def quitar_tildes(texto):
    """Normaliza un texto a minusculas sin tildes, para comparaciones robustas."""
    if not texto:
        return ""
...
def extraer_mw_del_detalle(detalle):
    """
    Suma TODAS las potencias en MW mencionadas en el texto del detalle.
    Busca patrones como '68 MW', '170.83 MW', '60.7 MW y 33.4 MW', '1,5 MW'.

    Devuelve la suma como float, o None si no encuentra ninguna.
    Ejemplo: "Chimay y Yanango con 60.7 MW y 33.4 MW" -> 94.1
    """
    if not detalle:
        return None
    # Encontrar TODOS los numeros (con punto o coma decimal) seguidos de 'MW'.
    encontrados = re.findall(r"(\d+(?:[.,]\d+)?)\s*MW", detalle, re.IGNORECASE)
    if not encontrados:
        return None
    total = 0.0
...
def detalle_menciona_central_propia(detalle):
    """
    Devuelve el nombre de la primera central propia mencionada en el detalle,
    o None si no menciona ninguna. Compara sin tildes ni mayus/minus.

    IMPORTANTE: busca cada central como PALABRA COMPLETA, no como substring.
    Asi evita falsos positivos: "Chimay" NO coincide dentro de "Cachimayo",
    "Rubi" NO coincide dentro de "Rubina", etc. Se consideran limites de palabra
    los espacios, puntuacion y bordes del texto.
    """
    if not detalle:
        return None
    texto = quitar_tildes(detalle)
    for central in CENTRALES_PROPIAS:
        clave = quitar_tildes(central)
        # \b = limite de palabra. re.escape por si el nombre tiene espacios
        # (ej. "santa rosa") u otros caracteres especiales.
...
def consultar_coes():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Procesar cada evento: abrir su detalle y aplicar las 3 reglas.
            #   COND 1: empresa = ORYGEN                          -> reportar
            #   COND 2: el detalle menciona una central propia    -> reportar
            #   COND 3: tercero, suma de MW del detalle >= umbral  -> reportar
            for ev in todos:
                es_orygen = EMPRESA_FILTRO.upper() in ev["empresa"].upper()

                # Abrimos el detalle SIEMPRE: para ORYGEN aporta info, y para
                # terceros es lo unico que permite ver centrales propias (COND 2)
                # y la suma de MW (COND 3).
                ev["detalle"] = extraer_detalle_popup(page, ev["_indice_fila"])
                ev["mw"] = extraer_mw_del_detalle(ev["detalle"])
                central_mencionada = detalle_menciona_central_propia(ev["detalle"])

                # COND 1 - ORYGEN por empresa.
                if es_orygen:
                    ev["motivo"] = "ORYGEN"
                ...
                # COND 2 - Otra empresa, pero el detalle menciona central propia.
                if central_mencionada:
                    ev["motivo"] = f"CENTRAL PROPIA ({central_mencionada})"
                    eventos_orygen.append(ev)
                    log(f"  -> CENTRAL PROPIA ({ev['empresa'][:18]}): "
                        f"menciona {central_mencionada}")
                    continue

                # COND 3 - Tercero, radar por suma de MW >= umbral.
                if REPORTAR_OTRAS_EMPRESAS and ev["mw"] is not None \
                        and ev["mw"] >= UMBRAL_MW:
                    ev["motivo"] = f">{UMBRAL_MW:.0f}MW"
                    eventos_orygen.append(ev)
                    log(f"  -> RELEVANTE ({ev['empresa'][:18]}): "
                        f"suma {ev['mw']} MW supera umbral")
                    continue

                # No cumple ninguna condicion -> se descarta.
                mw_txt = f"suma {ev['mw']} MW" if ev["mw"] is not None else "sin MW"
                log(f"  -> descartado ({ev['empresa'][:18]}): {mw_txt}")
...
def notificar_evento(ev):
    ...
    elif motivo.startswith("CENTRAL PROPIA"):
        # Extraer el nombre de la central de "CENTRAL PROPIA (Chimay)".
        cabecera = f"ALERTA COES - {motivo}"
    else:
